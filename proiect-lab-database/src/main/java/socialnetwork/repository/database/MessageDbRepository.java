...
public class MessageDbRepository implements PaginatedRepository<Long, Message> {
    ...
    public Iterable<Message> findAll() {
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
            ...
            while (resultSet.next())
            {
                ...
                Message m = new Message(message, from, reply);
                m.setDate(date);
                m.setId(id);
    ...
    public Message save(Message entity) {
        ...
        Long reply = entity.getReply();
        String date = entity.getDate().toString();


        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement("INSERT INTO messages(user_from,message,date_m,reply) VALUES (?,?,?,?)")) {

            statement.setLong(1, id_f);
            statement.setLong(4, reply);
