...
public class CommunityService extends ObservableClass {
    ...
    private PaginatedRepository<Long, Event> repositorye;
    ...
    public CommunityService(PaginatedRepository<Long, User> repository, PaginatedRepository<Long, Message> repositorym, PaginatedRepository<Long, FriendRequest> repositoryr, PaginatedRepository<Tuple<Long, Long>, Friendship> repositoryf,PaginatedRepository<Long, Event> repositorye) {
    ...
    public User deleteUser(User user)
    {
        ...
        for(Message x: messages)
            ...
            {
                Message message = new Message(x.getMessage(),x.getFrom(),x.getReply());
    ...
    public void setPage(Integer page)
    {
        ...
        repositorye.setPage(page);
        repositoryf.setPage(page);
        repositoryr.setPage(page);
        repositorym.setPage(page);
    }

    public Iterable<Event> getFirstPageEvents(User user)
    {
        return repositorye.getCurrentPage(user);
    }
    public Iterable<Event> getNextPageEvents(User user)
    {

        return repositorye.nextPage(user);
    }
    public Iterable<Event> getPreviousPageEvents(User user)
    {

        return repositorye.previousPage(user);
    }
    ...
    public Event createEvent(Event ev) throws Exception
    {
        try
        {

            ev=  repositorye.save(ev);
        }
        catch(Exception e)
        {
            throw new Exception(e.getMessage());
        }
        if(ev!=null) throw new IllegalArgumentException("Something stopped us from creating this !Please try again!");
        notify_observer();
        return ev;

    }

    public Event un_follow(Event ev,User u,Boolean interested) throws Exception
    {
        try
        {
            if(interested) ev.addParticipant(u.getId());
            else ev.deleteParticipant(u.getId());

            ev=  repositorye.update(ev);
        }
        catch(Exception e)
        {
            throw new Exception(e.getMessage());
        }
        if(ev!=null) throw new IllegalArgumentException("Something went wrong !Please try again!");
        notify_observer();
        return ev;
    }
   ...
   public Iterable<FriendRequest> restoreRequests()
    ...
    public Event findEvent(String accessibleText)
    {
        return repositorye.findOne((long) Integer.parseInt(accessibleText));
    }
    ...
    public ArrayList<Event> getMyEvents(User user)
    {
        ArrayList<Event> events = new ArrayList<>();
        for(Event e: repositorye.findAll())
        {
            if(e.getParticipants().contains(user.getId())||e.getCreator().equals(user.getId()))
                events.add(e);
        }
            return events;
    }
